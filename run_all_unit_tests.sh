...
run_test() {
    ...
    if [ -f "$test_file" ]; then
        ...
        if [[ "$test_file" == *.rs ]]; then
            # Rust文件，使用rust-script运行
            if command -v rust-script &> /dev/null; then
                if rust-script "$test_file"; then
                ...
                if rustc "$test_file" -o "$temp_exe" 2>/dev/null && "$temp_exe"; then
                    ...
                    echo -e "${YELLOW}⚠️  ${test_name} 跳过 (需要rust-script或rustc)${NC}"
                    rm -f "$temp_exe"
                fi
            fi
            ...
            echo -e "${YELLOW}⚠️  ${test_name} 跳过 (未知文件类型)${NC}"
        fi
    else
...
# 1. BSC核心适配单元测试
run_test "BSC核心适配单元测试" "test_bsc_unit_tests.rs" "测试L1Network枚举、BSCNetworkConfig结构体、EthConfig BSC优化等核心功能"

# 2. 网络验证器单元测试
run_test "BSC网络验证器单元测试" "test_network_validation_unit.rs" "测试BSC网络验证器的各种验证逻辑和错误处理"

# 3. 配置解析单元测试
run_test "BSC配置解析单元测试" "test_config_parsing_unit.rs" "测试TOML、JSON、环境变量等配置文件的解析和验证"

# 4. 现有的集成测试
if [ -f "test-bsc-core-adaptation.sh" ]; then
    run_test "BSC核心适配集成测试" "test-bsc-core-adaptation.sh" "BSC核心适配的集成测试"
fi

if [ -f "test-bsc-network-validation.py" ]; then
    run_test "BSC网络验证集成测试" "test-bsc-network-validation.py" "BSC网络验证逻辑的集成测试"
fi

if [ -f "test-rust-simple.sh" ]; then
    run_test "Rust代码编译测试" "test-rust-simple.sh" "Rust代码的编译和运行测试"
fi

if [ -f "test-project-compilation.sh" ]; then
    run_test "项目编译兼容性测试" "test-project-compilation.sh" "项目编译兼容性测试"
fi

if [ -f "verify-bsc-core-adaptation.sh" ]; then
    run_test "BSC核心适配验证测试" "verify-bsc-core-adaptation.sh" "BSC核心适配文件完整性验证"
fi
...
### 1. BSC核心适配单元测试
- **文件**: test_bsc_unit_tests.rs
- **描述**: 测试L1Network枚举、BSCNetworkConfig结构体、EthConfig BSC优化等核心功能
- **测试数量**: 22个测试用例
- **覆盖范围**: L1Network枚举、BSCNetworkConfig、EthConfig优化、边界条件、性能测试

### 2. BSC网络验证器单元测试
- **文件**: test_network_validation_unit.rs
- **描述**: 测试BSC网络验证器的各种验证逻辑和错误处理
- **测试数量**: 21个测试用例
- **覆盖范围**: 网络验证错误、BSC验证器、RPC连接、边界条件、集成测试

### 3. BSC配置解析单元测试
- **文件**: test_config_parsing_unit.rs
- **描述**: 测试TOML、JSON、环境变量等配置文件的解析和验证
- **测试数量**: 23个测试用例
- **覆盖范围**: TOML解析、JSON解析、环境变量解析、配置验证、错误处理
