...
use crate::{abstract_l1_interface::OperatorType, network_aware::NetworkType, EthSenderError};
...
/// Default fallback fee for BSC when RPC returns 0 (1 Gwei)
const BSC_DEFAULT_FEE_WEI: u64 = 1_000_000_000;
    ...
    fn assert_fee_is_not_zero(&self, value: u64, fee_type: &'static str) -> u64 {
        if value == 0 {
            match self.network_type {
                ...
                NetworkType::Bsc => {
                    tracing::warn!(
                        "BSC RPC returned 0 for {fee_type} fee, using fallback: {} wei ({} Gwei)",
                        BSC_DEFAULT_FEE_WEI,
                        BSC_DEFAULT_FEE_WEI / 1_000_000_000
                    );
                    BSC_DEFAULT_FEE_WEI
                }
                NetworkType::Other => {
                    let default_value = BSC_DEFAULT_FEE_WEI;
                    tracing::warn!(
                        "Non-Ethereum network: RPC returned 0 for {fee_type} fee, using fallback: {} wei ({} Gwei)",
                        default_value, default_value / 1_000_000_000
                    );
                    default_value
                }
            }
        ...
        if priority_fee_per_gas > self.max_acceptable_priority_fee_in_gwei {
            match self.network_type {
                ...
                NetworkType::Bsc | NetworkType::Other => {
                    tracing::warn!(
                        "priority_fee_per_gas {} exceeds max acceptable {}, capping",
                        priority_fee_per_gas,
                        self.max_acceptable_priority_fee_in_gwei
                    );
        ...
        if let Some(previous_sent_tx) = previous_sent_tx {
            ...
            match (blob_result, base_result) {
                ...
                (Err(_), Err(_)) => {
                    ...
                    blob_base_fee_per_gas = previous_sent_tx.blob_base_fee_per_gas.unwrap_or(0)
