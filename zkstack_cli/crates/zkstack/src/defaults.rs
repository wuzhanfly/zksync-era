...
pub fn generate_db_names(config: &ChainConfig) -> DBNames {
    let network_name = config
        ...
        .replace('-', "_");
    DBNames {
        server_name: format!("zksync_server_{}_{}", network_name, config.name),
        prover_name: format!("zksync_prover_{}_{}", network_name, config.name),
    }
}
...
pub fn generate_private_rpc_db_name(config: &ChainConfig) -> String {
    let network_name = config
        ...
        .to_ascii_lowercase()
        .replace('-', "_");
    format!("zksync_private_rpc_{}_{}", network_name, config.name)
}
...
pub fn generate_explorer_db_name(config: &ChainConfig) -> String {
    let network_name = config
        ...
        .to_ascii_lowercase()
        .replace('-', "_");
    format!("zksync_explorer_{}_{}", network_name, config.name)
}
...
pub fn generate_external_node_db_name(config: &ChainConfig) -> String {
    let network_name = config
        ...
        .to_ascii_lowercase()
        .replace('-', "_");
    format!("external_node_{}_{}", network_name, config.name)
}
