...
async fn apply_bsc_optimized_general_config(
    ...
    logger::success(&format!("✅ BSC优化配置已应用 (网络: {:?})", l1_network));
...
async fn apply_ethereum_optimized_general_config(
    ...
    let mut general_config: serde_yaml::Value =
        serde_yaml::from_str(&existing_content).with_context(|| "无法解析general.yaml文件")?;
    ...
    let optimized_content =
        serde_yaml::to_string(&general_config).with_context(|| "无法序列化优化后的配置")?;

    shell.write_file(&general_config_path, &optimized_content)?;

    logger::success(&format!("✅ 以太坊优化配置已应用 (网络: {:?})", l1_network));
    ...
    match l1_network {
        L1Network::Mainnet => {
            ...
            sender_config.insert(
                "max_acceptable_priority_fee_in_gwei".into(),
                serde_yaml::Value::Number(100_000_000_000u64.into()),
            ); // 100 Gwei
            sender_config.insert(
                "aggregated_block_commit_deadline".into(),
        ...
        L1Network::Sepolia | L1Network::Holesky => {
            ...
            sender_config.insert(
                "max_acceptable_priority_fee_in_gwei".into(),
                serde_yaml::Value::Number(50_000_000_000u64.into()),
            ); // 50 Gwei
            sender_config.insert(
                "aggregated_block_commit_deadline".into(),
...
fn apply_bsc_optimizations_inline(
    ...
    sender_config.insert(
        "max_txs_in_flight".into(),
        serde_yaml::Value::Number(50.into()),
    );
    sender_config.insert(
        "max_acceptable_priority_fee_in_gwei".into(),
        serde_yaml::Value::Number(5_000_000_000u64.into()),
    ); // 5 Gwei
    sender_config.insert(
        "aggregated_block_commit_deadline".into(),
    ...
    sender_config.insert(
        "pubdata_sending_mode".into(),
        serde_yaml::Value::String("BLOBS".to_string()),
    );

    // Watcher配置
    let watcher_config = eth_config
        .entry("watcher".into())
        .or_insert_with(|| serde_yaml::Value::Mapping(serde_yaml::Mapping::new()))
        .as_mapping_mut()
        .ok_or_else(|| anyhow::anyhow!("watcher配置格式错误"))?;

    watcher_config.insert(
        "confirmations_for_eth_event".into(),
        serde_yaml::Value::Number(2.into()),
    );
    watcher_config.insert(
        "eth_node_poll_interval".into(),
        serde_yaml::Value::Number(1500.into()),
    );

    // State Keeper配置
    let state_keeper_config = config_map
        .entry("state_keeper".into())
        .or_insert_with(|| serde_yaml::Value::Mapping(serde_yaml::Mapping::new()))
        .as_mapping_mut()
        .ok_or_else(|| anyhow::anyhow!("state_keeper配置格式错误"))?;

    state_keeper_config.insert(
        "block_commit_deadline_ms".into(),
        serde_yaml::Value::Number(3000.into()),
    );
    state_keeper_config.insert(
        "miniblock_commit_deadline_ms".into(),
    ...
    bsc_fee_config.insert(
        "min_base_fee_gwei".into(),
        serde_yaml::Value::Number(0.1.into()),
    );
    bsc_fee_config.insert(
        "max_base_fee_gwei".into(),
        serde_yaml::Value::Number(5.0.into()),
    );
    bsc_fee_config.insert(
        "target_base_fee_gwei".into(),
